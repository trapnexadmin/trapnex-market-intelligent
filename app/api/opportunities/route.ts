...
export async function GET(request:NextRequest){
  ...
  const results=await Promise.all(symbols.map(async symbol=>{
    try{
      ...
      const opportunity=aggregateProviderContext(symbol,context,historical.candles);
      return {
        symbol,opportunity,sources:context.sources,errors:context.errors,
        technical:{candleCount:historical.candles.length,entry:historical.entry,stopLoss:historical.stopLoss,target:historical.target},
      };
    }catch(error){
      return {
        symbol,opportunity:null,sources:[],
        errors:[error instanceof Error?error.message:"PROVIDER_ERROR"],
        technical:{candleCount:0,entry:null,stopLoss:null,target:null},
      };
    }
  }));

  const ranked=results
    .filter(item=>item.opportunity?.decision==="STRONG_CANDIDATE"||item.opportunity?.decision==="CANDIDATE")
  ...
  return NextResponse.json({
    ...
    ranked,
    rejected:results.filter(item=>!item.opportunity||item.opportunity.decision==="INSUFFICIENT_DATA").map(item=>item.symbol),
