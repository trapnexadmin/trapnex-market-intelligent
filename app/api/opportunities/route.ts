...
import { getUnifiedMarketContext } from "@/lib/opportunity/unified-context";
...
export async function GET(request: NextRequest) {
  ...
  const results = await Promise.all(
    symbols.map(async (symbol) => {
      try {
        ...
        const market = await getUnifiedMarketContext(symbol);
        ...
        const context = {
          ...
          sectorPulse: market.sectorPulse,
          sources: [
            ...new Set([
              ...live.context.sources,
              ...historical.sources,
              market.provenance.provider,
            ]),
          ],
          errors: [
            ...live.context.errors,
  ...
  const ranked = results
    .filter(
      (item) =>
        item.opportunity?.decision === "STRONG_CANDIDATE" ||
        item.opportunity?.decision === "CANDIDATE",
    )
    .sort(
      (a, b) =>
