...
export function calculateSectorPulse(sector:string,snapshots:MarketSnapshot[]):SectorPulse{
  const usable=snapshots.filter(s=>Number.isFinite(s.price)&&s.previousClose!==null&&Number.isFinite(s.previousClose));
  if(!usable.length){
    return {sector,score:null,direction:"INSUFFICIENT_DATA",sampleSize:0,calculatedAt:new Date().toISOString()};
  }
